...
[在线说明书](https://pokerjest.github.io/animateAutoTool/) • [功能特性](#-功能特性) • [快速开始](#-快速开始) • [使用说明](#-使用说明) • [开发](#-开发)
...
- 🎨 **现代 UI**：基于 Vue 3 + TypeScript + Tailwind CSS 的响应式动漫媒体库界面。
...
> [!NOTE]
> GitHub Actions 会同时产出带版本号的目录包、单文件 Windows `exe`，以及 macOS `dmg`，例如 `animate-server_v0.9.1_linux_amd64.tar.gz`、`animate-server_v0.9.1_windows_amd64.exe`、`animate-server_v0.9.1_darwin_arm64.dmg`。同时会产出 `SHA256SUMS.txt` 供应用内自动更新校验完整性使用。
...
- 在运行软件的电脑上打开 `http://localhost:8306`，点击“开始初始化”即可直接设置自己的管理员密码，无需输入随机密码。
- 这个免密入口只在首次启动、localhost 直连且初始化未完成时可用；远程和反向代理访问会被拒绝。
- 随机初始化凭据仍会保存在本机数据目录的 `data/bootstrap/admin.json` 作为 bootstrap 状态，且不会下发给浏览器。
...
项目内置敏感操作审计日志,自动记录登录、密码变更、删除订阅 / 本地目录、备份恢复、R2 删除、AI 设置变更等动作。

- **存储**:本地数据库 `audit_logs` 表(由 migration `004_audit_logs` 自动创建)
- **查询接口**：`GET /api/v1/audit-logs?action=login.failure&page_size=50`，字段说明见 [docs/api.md](docs/api.md)。
- **保留策略**:目前不会自动归档,如需定期清理可在 SQLite 上直接 `DELETE FROM audit_logs WHERE created_at < ...`

适合多人共享部署的事后追溯,**不**用于实时告警——若需要后者,建议把日志接入外部 SIEM。

### 8. 运行时监控说明

项目已内置轻量运行时监控与前端可视化，默认用于单机/个人场景：

- **后端接口**：`GET /api/v1/runtime`（需要登录态）
...
│   └── ...
├── web/             # 前端资源
│   ├── frontend/    # Vue 3 + TypeScript 源码
│   ├── dist/        # Vite 构建产物（嵌入 Go 二进制）
│   └── static/      # 本地图片资源
├── scripts/         # 运维脚本
└── config.yaml      # 配置文件
```
...
| **ORM** | GORM |
| **数据库** | SQLite |
| **前端** | Vue 3 + TypeScript + Tailwind CSS + Pinia + TanStack Query |
| **构建** | Node.js 22 + Vite + Go + GitHub Actions |
...
- [SECURITY.md](SECURITY.md):安全漏洞报告流程(请勿在公开 Issue 报告)
- [docs/architecture.md](docs/architecture.md):分层架构与约定

报告 Bug 或提议特性请用 [Issues](https://github.com/pokerjest/animateAutoTool/issues),已提供模板。
