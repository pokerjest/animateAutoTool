...
> [!NOTE]
> GitHub Actions 会同时产出带版本号的目录包、单文件 Windows `exe`，以及 macOS `dmg`，例如 `animate-server_v0.5.4_linux_amd64.tar.gz`、`animate-server_v0.5.4_windows_amd64.exe`、`animate-server_v0.5.4_darwin_arm64.dmg`。同时会产出 `SHA256SUMS.txt` 供应用内自动更新校验完整性使用。
