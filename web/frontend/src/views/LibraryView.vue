...
async function searchMatch(){if(!matchQuery.value.trim()&&!selected.value)return;try{await actions.run('search-match',async()=>{const query=matchQuery.value||selected.value?.title||'';const params=new URLSearchParams({q:query,source:matchSource.value});const sourceID=selectedSourceID();if(sourceID)params.set('source_id',String(sourceID));try{const result=await api<MetadataMatchSearchResult>(`/metadata/match-search?${params}`);if(Array.isArray(result?.candidates)){matchResults.value=result.candidates;matchStatus.value=result.source_status;return}}catch{/* older server/client compatibility fallback below */}const legacy=await api<MetadataSearchResult[]>(`/metadata/search?q=${encodeURIComponent(query)}&source=${matchSource.value}`);matchResults.value=legacy.map(item=>({title:item.name_cn||item.name,summary:item.summary,air_date:item.air_date,image:item.images?.large||'',score:1,evidence:['兼容旧版单源搜索'],[matchSource.value]:{id:item.id,source:matchSource.value as 'bangumi'|'tmdb'|'anilist',name:item.name,name_cn:item.name_cn,image:item.images?.large||'',summary:item.summary,air_date:item.air_date}} as MetadataMatchCandidate));matchStatus.value={}})}catch(e){ui.toast(e instanceof Error?e.message:'搜索失败','error')}}
async function fixMatch(item:MetadataMatchCandidate){if(!selected.value)return;const matches={bangumi_id:item.bangumi?.id||0,tmdb_id:item.tmdb?.id||0,anilist_id:item.anilist?.id||0};try{await actions.run(`fix-${item.bangumi?.id||item.tmdb?.id||item.anilist?.id}`,async()=>{await api('/library/fix-match',{method:'POST',body:JSON.stringify({id:selected.value!.ID,type:'metadata',matches}),headers:{'Content-Type':'application/json'}});ui.toast('三源匹配已更新');matchResults.value=[];qc.invalidateQueries({queryKey:['library']})})}catch(e){ui.toast(e instanceof Error?e.message:'匹配失败','error')}}
async function setBangumi(statusValue:number){if(!selected.value?.bangumi_id)return;try{await actions.run(`bangumi-${statusValue}`,async()=>{await api(`/bangumi/subject/${selected.value!.bangumi_id}/collection`,{method:'POST',body:JSON.stringify({status:statusValue}),headers:{'Content-Type':'application/json'}});ui.toast('Bangumi 收藏状态已同步')})}catch(e){ui.toast(e instanceof Error?e.message:'同步失败','error')}}
function open(item:LibraryItem){selected.value=item;matchQuery.value=item.title_cn||item.title;matchResults.value=[];matchStatus.value={}}
