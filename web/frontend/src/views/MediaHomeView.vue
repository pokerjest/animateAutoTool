...
<template>
  <div class="page-grid">
    <PageHeader eyebrow="MEDIA CENTER" title="媒体首页" description="从 Jellyfin 读取媒体库、继续观看和收藏内容。播放线路统一在系统设置中管理。">
    ...
    <section v-else-if="!connected" class="panel p-7">
      <StateBlock state="empty" title="还没有可用的 Jellyfin 媒体服务" description="请先在系统设置 → 媒体服务中配置 Jellyfin，然后回来刷新。" />
      <div class="mt-5 flex justify-center"><RouterLink class="btn btn-primary" to="/settings">打开媒体服务设置</RouterLink></div>
    </section>
    <template v-else>
      ...
      <form class="panel flex gap-3 p-4" @submit.prevent="submitSearch">
        <label class="search-field flex-1"><Search :size="18" /><input v-model="search" class="field field-leading-icon" placeholder="搜索媒体标题、剧集或电影" aria-label="搜索媒体" /></label>
