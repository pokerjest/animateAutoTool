...
describe('background task buttons', () => {
  ...
  it('tracks local scans independently from the request lifetime', async () => {
    ...
    tasks.upsert({ id: 'local-scan', kind: 'scan', title: '本地扫描', detail: '正在扫描文件夹 3/9：Anime/Show', current: 3, total: 10, tone: 'running', updatedAt: new Date().toISOString() })
    await flushPromises()
    expect(wrapper.text()).toContain('正在扫描文件夹 3/9')
    expect(wrapper.text()).toContain('30%')
    expect(wrapper.text()).toContain('3 / 10 个扫描步骤')

    tasks.upsert({ id: 'local-scan', kind: 'scan', title: '本地扫描', detail: '正在扫描文件夹 465/465', current: 465, total: 466, tone: 'running', updatedAt: new Date(Date.now()+1000).toISOString() })
    await flushPromises()
    expect(wrapper.text()).toContain('99%')
    expect(wrapper.text()).not.toContain('100%')

    tasks.upsert({ id: 'local-scan', kind: 'scan', phase: 'metadata', title: '本地扫描', detail: '正在整理元数据 1/8', current: 1, total: 8, tone: 'running', updatedAt: new Date(Date.now()+2000).toISOString() })
    await flushPromises()
    expect(wrapper.text()).toContain('第 2/2 阶段 · 元数据整理')
    expect(wrapper.text()).toContain('1 / 8 部本地番剧')

    tasks.upsert({ id: 'local-scan', kind: 'scan', title: '本地扫描', detail: '扫描完成', tone: 'success', updatedAt: new Date(Date.now()+3000).toISOString() })
    await flushPromises()
