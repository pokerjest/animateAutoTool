...
func (s *LocalAnimeStore) ListForSubscriptionMatching() ([]model.LocalAnime, error) {
	...
	query := s.db.Model(&model.LocalAnime{}).
		Select("local_animes.id", "local_animes.title", "local_animes.path", "local_animes.metadata_id", "local_animes.jellyfin_series_id").
	...
	if err := query.Preload("Metadata", func(tx *gorm.DB) *gorm.DB {
		return tx.Select(
			...
			"bangumi_title", "tmdb_title", "ani_list_title", "bangumi_id",
		)
