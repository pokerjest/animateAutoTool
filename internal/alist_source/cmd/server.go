...
// StartManually starts the server with a specific data directory, bypassing CLI flags
func StartManually(dataDir string) {
	if dataDir != "" {
		flags.DataDir = dataDir
	}

	// Manual Init Sequence mirroring ServerCmd.Run but without Cobra
	Init()
	if conf.Conf.DelayedStart != 0 {
		utils.Log.Infof("delayed start for %d seconds", conf.Conf.DelayedStart)
		time.Sleep(time.Duration(conf.Conf.DelayedStart) * time.Second)
	...
	// Force Release mode for embedded
	gin.SetMode(gin.ReleaseMode)
	...
	// Hold main thread? No, let's let it run in background.
	// Wait, the original Run() blocks at the end. We should probably block if run in a goroutine suitable for `StartAlistServer`.
	// But for `StartManually`, we might want it to return so caller can manage?
	// Current usage in my service.go is `go func() ...` so blocking is fine.

	// We'll skip the elaborate signal handling for now and just block select{} or similar if we want to keep it alive via this function.
	// However, the HTTP server `ListenAndServe` is in a `go func` in the original code?
	// Yes lines 61-66.
	// And it waits on `wg.Wait()` at the end after waiting for signals.

	// Let's just block on select{} to keep it running
	select {}
}
